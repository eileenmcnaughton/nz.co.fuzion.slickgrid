...
function civicrm_api3_slick_grid_create($params) {
  ...
  $exists = CRM_Core_DAO::singleValueQuery(" SELECT count(*) FROM $tempTable WHERE id = " . $params['id']);
  if($exists) {
    ...
    $sql = " INSERT INTO $tempTable (id, ". str_replace('-', '__', implode(',', array_keys($fields))) . ') '
...
function civicrm_api3_slick_grid_get($params) {
  ...
  while($dao->fetch()) {
    ...
    foreach ($dao as $field => $value) {
      ...
      $values[$dao->id][str_replace('__', '-', $field)] = $value;
    }
  }
